...
- **Текущая версия приложения:** 4.5.5 (versionCode 13)
...
1. **AllowedIPs:** удаляется `::/0` всегда; остальные IPv6-префиксы — если у интерфейса нет IPv6-адреса; если список пуст — `0.0.0.0/0`. Причина: маршрутизация IPv6 в туннель без v6-адреса на интерфейсе = чёрная дыра = «VPN блокирует интернет» (главный баг сентября 2026).
